...
const drain = (err) => { queue.forEach(cb => cb(err)); queue = []; };
...
api.interceptors.response.use(
  ...
  async err => {
    ...
    orig._retry = true;
    if (refreshing) {
      return new Promise((res, rej) => queue.push(e => e ? rej(e) : res(api(orig))));
    }
    refreshing = api.post('/auth/refresh').then(() => {
      drain(null); refreshing = null;
    }).catch(e => {
      drain(e); refreshing = null;
      redirectToLogin();
